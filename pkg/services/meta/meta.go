...
func joinCluster(addr string) error {
	mc := conf.CurrentNode().Meta

	query := url.Values{"id": {conf.NodeID()}, "addr": {mc.RaftAddr}}
	if mc.RaftVoter {
...
func createRaftSnapshotStore(mc *conf.MetaConf) (raft.SnapshotStore, error) {
...
func createRaftStore(mc *conf.MetaConf) (raft.LogStore, raft.StableStore, error) {
	switch mc.RaftStore {
	case "memory":
